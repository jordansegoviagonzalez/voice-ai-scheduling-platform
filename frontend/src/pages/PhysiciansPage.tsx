      ...
      <section className="physician-grid">
        {query.data.doctors.map((doctor) => (
          <article className="physician-card" key={doctor.id}>
            <div className="physician-head"><div className="doctor-avatar">{doctor.first_name[0]}{doctor.last_name[0]}</div><div><h2>{doctor.full_name}</h2><p>{doctor.primary_specialty}</p><span className={doctor.accepts_new_patients ? 'accepts-new' : 'returning-only'}>{doctor.accepts_new_patients ? 'Accepts new patients' : 'Returning patients only'}</span>{doctor.is_general_orthopedics ? <span className="capability-chip">General Orthopedics</span> : null}</div></div>
